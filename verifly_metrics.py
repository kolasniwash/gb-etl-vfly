...
def data_processing(data):
    ...
    # drop columns
    data = data.drop(['Unnamed: 0', 'fb_campaign_group_name', 'fb_campaign_name', 'fb_adgroup_name'], axis=1)
   ...
   # Adding cohort group
    ...
    # Revenue from cents to euros
    data['revenue'] = data['revenue'].fillna(0)
    data['revenue'] = round(data['revenue'] / 100, 2)
...
def total_revenue_cohort(data):
    ...
    grouping = ['CohortGroup', 'random_user_id', 'tracker' ,'network_name', 'campaign_name', 'adgroup_name', 'creative_name', 'region', 'os_name']

    grouped_data = data.groupby(grouping)

    # count the unique users, orders, and total revenue per Group + Period
    cohorts = grouped_data.agg({'revenue': 'sum'})

    # make the column names more meaningful
    cohorts.rename(columns={'revenue': 'CohortRevenue'}, inplace=True)
    cohorts = cohorts.reset_index()
    
    cohorts = cohorts.drop('random_user_id', axis=1)
    return cohorts

def merge_data(agg_metrics, cohort_data):
    
    grouping = ['date', 'tracker', 'network_name', 'campaign_name', 'adgroup_name', 'creative_name', 'region', 'os_name']
    cohort_data.rename(columns={"CohortGroup": "date"})

    merged_data = agg_metrics.merge(cohort_data, how='left', on=grouping)
    return merged_data
